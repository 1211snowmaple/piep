...
//! # 測った結果（2026-08-29）
//!
//! 棚全体（163,393 断片・150 作品）で、現行と 30m:
//!
//! ```text
...
//! ```
//!
//! 130m は棚全体を通すのが重すぎるので、24,005 断片へ絞って三つ並べた
//! （`PIEP_PROBE_CHUNK_CAP=24000`。棚が小さいぶん、どれも甘く出ている）:
//!
//! ```text
//! ■ 題名を問いにしたとき            1位     5位内   10位内   MRR
//!   multilingual-e5-small (384)    88.7%   99.3%  100.0%   0.939
//!   ruri-v3-30m (256)              84.0%   94.0%   95.3%   0.892
//!   ruri-v3-130m (512)             80.7%   93.3%   95.3%   0.861
//!
//! ■ 本文の一文を問いにしたとき
//!   multilingual-e5-small (384)    81.3%   92.0%   94.7%   0.864
//!   ruri-v3-30m (256)              71.3%   82.7%   84.7%   0.769
//!   ruri-v3-130m (512)             72.7%   87.3%   89.3%   0.791
//! ```
//!
//! **公表値と逆に出た。** JMTEB の検索課題では 30m が +10.8、130m が +14.6 と
//! されていて、乗り換える前提で調べ始めた。この棚では現行が両方で勝ち、しかも
//! **130m は 30m より良くもない**（題名では負けている）。順位表の序列がまるで
//! 再現しない。
//!
//! 速さも桁が違う。同じ 24,005 断片で 30m は 39 秒、130m は 368 秒（65 断片/秒）。
//! 棚全体なら 30m が 6 分、130m は 40 分を超える。
//!
//! 一般の検索課題と「自分の棚から、覚えている一文で作品を引く」は別物だという
//! ことで、**順位表では決められない**という結論そのものが、この道具の成果である。
//! 次の候補を見るときも、ここへ足して測ってから決めること。
