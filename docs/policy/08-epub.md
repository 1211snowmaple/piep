...
二段構えにしている。**内蔵の検証器は自分で書いた規則しか知らない**ので、外の
権威の代わりにはならない。逆に、外へ持っていく前に落とせるものを毎回 Java の
起動に付き合わせる理由も無い。

- **内蔵の検証器**（`epub/validate.rs`）— 組み立てた本をその場で見る。敵対的な
  メタデータ、全ての組み込みテンプレート、設定違い、コレクションの結合を
  `cargo test` が毎回通す
- **EPUBCheck 5.3.0** — `cargo run --example epub_conformance` が利用者の
  ライブラリを要さない原稿から組み込みテンプレートごとに1冊ずつ組み立て、CI が
  そのすべてを EPUBCheck に掛ける。**警告も失敗として扱う**

原稿は「通れば嬉しい」ものではなく、壊れるとしたらどこかで選んである。
エスケープの要る文字、ファイル名に使えない文字、二階層の目次、シリーズと
訳語つきタグ、閲覧制限の印。現在は fatal 0 / error 0 / warning 0 / usage 0。

実データから生成した本での確認は、手で行う。取得元の応答は手元にしか無いので
CI では再現できない。
